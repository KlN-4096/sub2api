export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexClientVersionPlaceholder: '如 0.146.1；留空则跟随自动同步',
        ...
        openaiCodexClientType: 'Codex 客户端身份',
        openaiCodexClientTypeLabel: '客户端类型',
        openaiCodexClientTypeCli: 'CLI',
        openaiCodexClientTypeDesktop: 'Desktop',
        openaiCodexClientTypeHint: '出站统一模拟的 Codex 客户端类型，保存后即时生效（无需重启）。CLI 跟随 GitHub Releases 的 0.x 版本；Desktop 模拟真实抓包形态 UA（首段为内嵌 CLI 版本），App 版本号（26.x 形态）走 Sparkle appcast 自动同步。',
        openaiCodexDesktopVersionSyncedValue: "Desktop 生效版本：{cliVersion}{'|'}{appVersion}（位置 1 内嵌 CLI 版本填 UA 首段 + version 头，GitHub alpha 自动同步 · 位置 2 App 版本填 UA 尾组，appcast 同步）",
        openaiCodexVersionGuideTitle: '版本号说明',
        openaiCodexVersionGuideCliReason: '首段、version 头与尾部标识组共用同一个版本号，自动同步或手填后三处一并覆盖，UA 只需保留 OS / 架构 / 终端指纹。',
        openaiCodexVersionGuideDesktopReason: '真实客户端同时声明两个版本：位置 1 是内嵌 CLI 快照（跟随 GitHub 最新 alpha prerelease 自动同步，hotfix 后缀可能有偏差、可覆写），位置 2 是 App 版本（appcast 自动同步）；留空各走同步值或兜底。',
        openaiCodexDesktopDualVersionFormat: "填写格式：CLI版本{'|'}App版本（可只填位置 1）",
        openaiCodexVersionGuideCliFormat: '填写格式：单个版本号（留空跟随自动同步）',
        openaiCodexClientVersionDesktopPlaceholder: "CLI版本{'|'}App版本，如 0.154.0-alpha.6.2{'|'}26.908.70816（可只填位置 1）",
        openaiCodexPreviewTitle: '出站 UA 预览',
        openaiCodexPreviewHint: '按当前表单值实时拼出出站身份的最终形态。版本段始终由生效版本重建（面板 UA 只贡献客户端名与 OS / 架构 / 终端指纹），与后端组装规则一致。',
        openaiCodexPreviewIdentityHeader: '身份头',
        openaiCodexPreviewUAHeader: 'User-Agent',
        openaiCodexPreviewOriginatorHeader: 'Originator',
        openaiCodexPreviewCli: 'CLI 模式',
        openaiCodexPreviewDesktop: 'Desktop 模式',
        openaiCodexPreviewCurrentLabel: '当前生效',
