export default {
    accounts: {
      ...
      fallbackActive: '已回退',
      cprOutbound: 'CPR 出口',
      cprOutboundHint: '实际出口由 CPR 决定：{endpoint}。上面这个代理只作用于 sub2api → CPR 这一跳。',
      ...
      openai: {
        ...
        turnStateDeprecated:
          '已废弃：2026-09-21 起，把 292 注入请求已经换不来正常服务。下面的自动接管、手填覆写、292 猎手（含降智时暂停调度）、降智恢复探测都已失效，只保留不维护，后续几个版本会移除。账号列表和用量记录里的 Turn-State 读数（入站 / 出站两列）不受影响，会一直保留。',
        ...
        turnStatePool: {
          ...
          hunterReady: '待命',
          hunterProbing: '探测中',
          recoverySummary: '恢复探测(已废弃) {streak}/{target} · {next}',
