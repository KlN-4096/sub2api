export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentDesktopPlaceholder: 'Codex Desktop/0.154.0-alpha.6.2 (Mac OS 26.5.2; arm64) unknown (Codex Desktop; 26.908.70816)',
        ...
        openaiCodexClientVersionPlaceholder: 'e.g. 0.146.1; leave empty to follow auto-sync',
        ...
        openaiCodexClientType: 'Codex client identity',
        openaiCodexClientTypeLabel: 'Client type',
        openaiCodexClientTypeCli: 'CLI',
        openaiCodexClientTypeDesktop: 'Desktop',
        openaiCodexClientTypeHint: 'The Codex client identity to emulate for all outbound requests. Takes effect immediately on save (no restart needed). CLI follows the 0.x versions from GitHub Releases; Desktop emulates the real captured UA form (leading segment is the embedded CLI version) with the app version (26.x form) auto-synced via the Sparkle appcast.',
        openaiCodexDesktopVersionSyncedValue: "Desktop effective versions: {cliVersion}{'|'}{appVersion} (position 1, the embedded CLI version, fills the UA leading segment + version header, auto-synced from GitHub alphas · position 2, the app version, fills the UA trailer group — appcast-synced)",
        openaiCodexVersionGuideTitle: 'Version guide',
        openaiCodexVersionGuideCliReason: 'The leading segment, the version header, and the trailer group share one version number — auto-sync or a manual value overwrites all three at once; the UA only needs to keep the OS / arch / terminal fingerprint.',
        openaiCodexVersionGuideDesktopReason: 'A real client declares two versions: position 1 is the embedded CLI snapshot (auto-synced from the latest GitHub alpha prerelease; hotfix suffixes may drift — override to pin), position 2 the app version (auto-synced via appcast); leave empty and each falls back to its synced value or fallback.',
        openaiCodexDesktopDualVersionFormat: "Input format: cliVersion{'|'}appVersion (position 1 alone is fine)",
        openaiCodexVersionGuideCliFormat: 'Input format: a single version (leave empty to follow auto-sync)',
        openaiCodexClientVersionDesktopPlaceholder: "cliVersion{'|'}appVersion, e.g. 0.154.0-alpha.6.2{'|'}26.908.70816 (position 1 alone is fine)",
        openaiCodexPreviewTitle: 'Outbound UA preview',
        openaiCodexPreviewHint: 'Builds the final outbound identity live from the current form values. The version segment is always rebuilt from the effective version (the panel UA only contributes the client name and OS / arch / terminal fingerprint), matching the backend assembly rules.',
        openaiCodexPreviewIdentityHeader: 'Identity headers',
        openaiCodexPreviewUAHeader: 'User-Agent',
        openaiCodexPreviewOriginatorHeader: 'Originator',
        openaiCodexPreviewCli: 'CLI mode',
        openaiCodexPreviewDesktop: 'Desktop mode',
        openaiCodexPreviewCurrentLabel: 'currently active',
